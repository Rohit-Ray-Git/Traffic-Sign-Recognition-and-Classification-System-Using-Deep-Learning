...
def classify(file_path):
    ...
    try:
        ...
        pred = model.predict(image)  # Predict class probabilities
        sign_class = classes[np.argmax(pred) + 1]  # Get class label
        label.configure(foreground='#011638', text=f"Prediction: {sign_class}")
    except Exception as e:
        label.configure(foreground='red', text=f"Error: {str(e)}")
